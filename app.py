    ...
    if repo:
        ...
        if not data_list:
            ...
            if selected_key:
                ...
                # 組合 GitHub Raw 圖片連結
                # 注意：為了避免快取，可以加上隨機參數，但在這裡我們直接用路徑
                repo_full_name = st.secrets["github"]["repo_name"]
                branch = st.secrets["github"]["branch"]
                img_url = f"https://raw.githubusercontent.com/{repo_full_name}/{branch}/{item['img_path']}"
                ...
                st.markdown(f"""
                    <div class="display-box">
                        ...
                        <img src="{img_url}" class="poke-img-style">
                    </div>
                """, unsafe_allow_html=True)

                # 聲音播放器
                if 'audio_path' in item and item['audio_path']:
                    audio_url = f"https://raw.githubusercontent.com/{repo_full_name}/{branch}/{item['audio_path']}"
                    st.audio(audio_url)
            ...
            if not repo:
                ...
                if any(d['id'] == new_id for d in current_data):
                    ...
                    try:
                        ...
                        st.error(f"發生未知錯誤: {e}")import streamlit as st
from github import Github
import json
import time
import base64
